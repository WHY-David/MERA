...
def scaling_dims(layer: Layer):
    ...
    vals = LAs.eigs(superop, k=10, which='LM', return_eigenvectors=False)
    dims = np.sort(-np.log2(np.abs(vals))/2)
...
def init_model(g=1., chi=6):
    ...
    return MERA(hAB, hBA, chi=chi, num_trans=1)
...
if __name__ == '__main__':
    ...
    # model = init_model(g=1., chi=6)
    # model.optimize(2000)
    # model.expand(chi=8, num_trans=1)
    # model.optimize(2000)
    # model.expand(chi=10, num_trans=2)
    # model.optimize(1800)
    # model.expand(chi=12, num_trans=2)
    # model.optimize(1800)
    # model.expand(chi=16, num_trans=2)
    # model.optimize(4000)
    ...
    dims = scaling_dims(model.si_layer)
    print(dims)
